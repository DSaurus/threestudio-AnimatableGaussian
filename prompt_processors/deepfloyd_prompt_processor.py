...
class DeepFloydPromptProcessor(PromptProcessor):
    ...
    def configure_text_encoder(self) -> None:
        ...
        # text_encoder = T5EncoderModel.from_pretrained(pretrained_model_name_or_path,subfolder="text_encoder",load_in_8bit=True,variant="8bit",device_map="auto",)
        self.pipe = IFPipeline.from_pretrained(
            self.cfg.pretrained_model_name_or_path,
            text_encoder=self.text_encoder,  # pass the previously instantiated 8bit text encoder
            unet=None,
        )
        # pipe = IFPipeline.from_pretrained(pretrained_model_name_or_path, text_encoder=text_encoder, unet=None)

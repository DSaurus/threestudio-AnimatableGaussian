...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        if self.fusion == "sum":
            ...
            self.fusion_conv = nn.Conv2d(
                block_out_channels[self.copy_first_n_block - 1] * (self.branch_num + 1),
                block_out_channels[self.copy_first_n_block - 1],
                kernel_size=3,
                padding=1,
            )
        else:
        ...
        if addition_embed_type == "text":
            ...
            if off_wa:
                ...
                size_cond_num = 8
            self.add_embedding = TimestepEmbedding(
                addition_time_embed_dim * size_cond_num,
                time_embed_dim,
                act_fn=act_fn,
                post_act_fn=timestep_post_act,
                cond_proj_dim=time_cond_proj_dim,
    ...
    def forward(
        ...
        for j in range(self.branch_num):
            for i, downsample_block in enumerate(self.down_blocks_branch[j]):
                if (
                    ...
                    sample_in_list[j], res_samples = downsample_block(
                        hidden_states=sample_in_list[j], temb=emb
                    )
        ...
        for i, upsample_block in enumerate(self.up_blocks[: -self.copy_last_n_block]):
            ...
            for j in range(self.branch_num):
                down_block_res_samples_list[j] = down_block_res_samples_list[j][
                    : -len(upsample_block.resnets)
                ]
        ...
        for j in range(self.branch_num):
            ...
            for i, upsample_block in enumerate(self.up_blocks_branch[j]):
                ...
                res_samples = down_block_res_samples_list[j][
                    -len(upsample_block.resnets) :
                ]
                down_block_res_samples_list[j] = down_block_res_samples_list[j][
                    : -len(upsample_block.resnets)
